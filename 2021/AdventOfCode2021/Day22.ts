...
function calculateSum(cubes: Cube[]) {
    let sum = 0;
...
function intersects(cube1: Cube, cube2: Cube): boolean {
    const [[x11, y11, z11], [x12, y12, z12]] = cube1;
    const [[x21, y21, z21], [x22, y22, z22]] = cube2;

    if (x11 > x22 || y11 > y22 || z11 > z22) {
        return false;
    }
    if (x12 < x21 || y12 < y21 || z12 < z21) {
        return false;
    }
    return true;
}

function isPointInCube(cube: Cube, point: [number, number, number]): boolean {
    const [[x1, y1, z1], [x2, y2, z2]] = cube;
    const [xp, yp, zp] = point;
    if (xp >= x1 && xp <= x2 && yp >= y1 && yp <= y2 && zp >= z1 && zp <= z2) {
        return true;
    }
    return false;
}

function getEdgeCoordinates(cube: Cube): [[number, number, number], [number, number, number]][] {
    const [[x1, y1, z1], [x2, y2, z2]] = cube;
    return [
        [[x1, y1, z1], [0, 0, 0]],
        [[x1, y1, z2], [0, 0, 1]],
...
function isTotalInCube(cube1: Cube, cube2: Cube): boolean {
    const [[x11, y11, z11], [x12, y12, z12]] = cube1;
    const [[x21, y21, z21], [x22, y22, z22]] = cube2;
    return x21 >= x11 && x22 <= x12 && y21 >= y11 && y22 <= y12 && z21 >= z11 && z22 <= z12;
}

function splitSinglePoint(cube: Cube, point: [[number, number, number], [number, number, number]]): Cube[] {
    console.log('split single point', cube, point);
    const [[x1, y1, z1], [x2, y2, z2]] = cube;
    const [[xp, yp, zp], [xo, yo, zo]] = point;
    const blocks: Cube[] = [];
    let xr1 = 0;
    let xr2 = 0;
    let yr1 = 0;
    let yr2 = 0;
    if (xo == 0) {
        blocks.push([[x1, y1, z1], [xp - 1, y2, z2]]);
        xr1 = xp;
        xr2 = x2;
    } else {
        if (xp + 1 <= x2) {
            blocks.push([[xp + 1, y1, z1], [x2, y2, z2]]);
        }
        xr1 = x1;
        xr2 = xp;
    }
    if (yo == 0) {
        blocks.push([[xr1, y1, z1], [xr2, yp - 1, z2]]);
        yr1 = yp;
        yr2 = y2;
    } else {
        if (yp + 1 <= y2) {
            blocks.push([[xr1, yp + 1, z1], [xr2, y2, z2]]);
        }
        yr1 = y1;
        yr2 = yp;
    }
    if (zo == 0) {
        blocks.push([[xr1, yr1, z1], [xr2, yr2, zp - 1]]);
    } else {
        if (zp + 1 <= z2) {
            blocks.push([[xr1, yr1, zp + 1], [xr2, yr2, z2]]);
        }
    }
    console.log(blocks);
    return blocks;
...
function splitDoublePoint(cube: Cube, points: [[number, number, number], [number, number, number]][]): Cube[] {
    console.log('split double point', cube, points[0], points[1]);
    const [[x1, y1, z1], [x2, y2, z2]] = cube;
    const [[xp1, yp1, zp1], [xo1, yo1, zo1]] = points[0];
    const [[xp2, yp2, zp2], [xo2, yo2, zo2]] = points[1];
    let halfCube: Cube | null;
    let restCube: Cube;
    if (xp1 != xp2) {
        console.log('split x');
        if (xp1 > xp2) {
            throw 'expected xp1 to be smaller'
        }
        if (xp1 < x1) {
            throw 'error expected it to be larger'
        }
        halfCube = [[x1, y1, z1], [xp1 - 1, y2, z2]]
        restCube = [[xp1, y1, z1], [x2, y2, z2]]
        if (xp1 == x1) {
            halfCube = null;
        }
    } else if (yp1 != yp2) {
        console.log('split y');
        if (yp1 > yp2) {
            throw 'expected yp1 to be smaller'
        }
        if (yp1 < y1) {
            throw 'error expected it to be larger'
        }
        halfCube = [[x1, y1, z1], [x2, yp1 - 1, z2]]
        restCube = [[x1, yp1, z1], [x2, y2, z2]]
        if (yp1 == y1) {
            halfCube = null;
        }
    } else if (zp1 != zp2) {
        ...
        halfCube = [[x1, y1, z1], [x2, y2, zp1 - 1]]
        restCube = [[x1, y1, zp1], [x2, y2, z2]]
        if (zp1 == z1) {
            halfCube = null;
        }
    } else {
        //every point is the same ... just split the cube as is with second point
        console.warn('double point merge');
        halfCube = null
        restCube = [[x1, y1, z1], [x2, y2, z2]]
    }
    console.log('half cube', halfCube);
    console.log('rest cube', restCube);
    let restSplit = splitSinglePoint(restCube, points[1]);
    if (halfCube != null) {
        return [halfCube, ...restSplit];
    }
    return restSplit;
}

function splitInside(cube: Cube, points: [[number, number, number], [number, number, number]][]): Cube[] {
    const [[x1, y1, z1], [x2, y2, z2]] = cube;
    const [[xp1, yp1, zp1], [xo1, yo1, zo1]] = points[0];
    let halfCube: Cube | null;
    let restCube: Cube;
    ...
    halfCube = [[x1, y1, z1], [xp1 - 1, y2, z2]];
    restCube = [[xp1, y1, z1], [x2, y2, z2]];
    restPoints = points.filter(([[x]]) => x != xp);
    if (xp1 < x1) {
        throw 'xp1 < x1';
    }
    if (xp1 == x1) {
    ...
    if (restPoints.length === 0) {
        //all x coordinates are the same so we slice it
        console.warn('slicing inside array');
        restPoints = points.slice(4);
    }
    else if (restPoints.length !== 4) {
        throw 'expect four points to remain';
    }
    console.log('halfCube', halfCube);
    console.log('restCube', restCube);
    console.log('restpoints', restPoints);
    let restSplit = splitQuadriplePoint(restCube, restPoints);
    let result: Cube[];
    if (halfCube != null) {
        result = [halfCube, ...restSplit];
    } else {
        result = restSplit;
    }
...
function splitQuadriplePoint(cube: Cube, points: [[number, number, number], [number, number, number]][]): Cube[] {
    console.log('cube', cube);
    console.log('points', points);
    const [[x1, y1, z1], [x2, y2, z2]] = cube;
    const [[xp1, yp1, zp1], [xo1, yo1, zo1]] = points[0];
    const [[xp2, yp2, zp2], [xo2, yo2, zo2]] = points[1];
    const [[xp3, yp3, zp3], [xo3, yo3, zo3]] = points[2];
    const [[xp4, yp4, zp4], [xo4, yo4, zo4]] = points[3];
    let halfCube: Cube | null;
    let restCube: Cube;
    let restPoints = [];
    if (xp1 == xp2 && xp1 == xp3 && xp1 == xp4) {
        console.log('split quadruple y');
        //x are all the same, split for y
        let yp = Math.min(yp1, yp2, yp3, yp4);
        halfCube = [[x1, y1, z1], [x2, yp - 1, z2]];
        restCube = [[x1, yp1, z1], [x2, y2, z2]];
        if (points.every(([[_, y]]) => y == yp)) {
            console.warn('special case for mulit reduce')
            restPoints = points.slice(2);
        } else {
            restPoints = points.filter(([[_, y]]) => y != yp);
        }
        if (yp1 < y1) {
            throw 'yp1 < y1';
        }
        ...
        console.log('split quadruple x');
        //x are different, split for x
        let xp = Math.min(xp1, xp2, xp3, xp4);
        halfCube = [[x1, y1, z1], [xp1 - 1, y2, z2]];
        restCube = [[xp1, y1, z1], [x2, y2, z2]];
        restPoints = points.filter(([[x]]) => x != xp);
        if (xp1 < x1) {
    ...
    if (restPoints.length !== 2) {
        console.error('rest points', restPoints);
        throw 'expect two points to remain';
    }
    let restSplit = splitDoublePoint(restCube, restPoints);
    let result: Cube[];
    if (halfCube != null) {
        result = [halfCube, ...restSplit];
    } else {
...
function getIntersectionOn(cube1: Cube, cube2: Cube): Cube[] {
    const [[x11, y11, z11], [x12, y12, z12]] = cube1;
    const [[x21, y21, z21], [x22, y22, z22]] = cube2;

    if (isTotalInCube(cube1, cube2)) {
        return [];
    }

    let edgeCoordinates = getEdgeCoordinates(cube1).map(([[x, y, z], [xo, yo, zo]]) => {
        let xn = xo == 0 ? (x < x21 ? x21 : x) : (x > x22 ? x22 : x);
        let yn = yo == 0 ? (y < y21 ? y21 : y) : (y > y22 ? y22 : y);
        let zn = zo == 0 ? (z < z21 ? z21 : z) : (z > z22 ? z22 : z);
        return [[xn, yn, zn], [xo, yo, zo]] as [[number, number, number], [number, number, number]];
    });
    let inCube2 = edgeCoordinates.filter(([p, _]) => isPointInCube(cube2, p));
    if (inCube2.length === 1) {
        console.log('single point split');
        return splitSinglePoint(cube2, inCube2[0])
    } else if (inCube2.length === 2) {
        console.log('double point split');
        return splitDoublePoint(cube2, inCube2)
    } else if (inCube2.length === 4) {
        console.log('quadrupel point split');
        console.log('cube1', cube1);
        return splitQuadriplePoint(cube2, inCube2);
    } else if (inCube2.length === 8) {
        console.log('inside point split');
        return splitInside(cube2, inCube2);
    } else if (inCube2.length === 0) {
        console.log(cube1);
        console.log(cube2);
        throw 'todo even worse';
        return [];
    } else {
        console.error('cube1', cube1)
        console.error('cube2', cube2)
        console.error('inCube', inCube2)
...
function isLit(cubes: Cube[], point: [number, number, number]): number {
    if (cubes.some(cube => isPointInCube(cube, point))) {
        return 1;
    }
    return 0;
}

function diffResults(arr: number[][][], cubes: Cube[]): boolean {
    let errors = [];
    for (let x = 0; x < 101; x++) {
        for (let y = 0; y < 101; y++) {
            for (let z = 0; z < 101; z++) {
                const lit = isLit(cubes, [x - 50, y - 50, z - 50])
                const expected = arr[x][y][z];
                if (lit != expected) {
                    errors.push(`expected ${x - 50},${y - 50},${z - 50} to be ${expected}, but was ${lit}`);
                }
            }
        }
    }
    if (errors.length > 0) throw errors.slice(0, 100).join('\r\n');
    return true;
}

function part1(input: string[]) {
    let commands = input.map((str) => {
        let mode = str.startsWith("on") ? "on" : "off";
        let rest = str.slice(3).split(",").map(s => s.split("=")[1].split(".."))
        let [[x1, x2], [y1, y2], [z1, z2]] = rest;
        return [mode, [+x1, +y1, +z1], [+x2, +y2, +z2]] as const;
    });
    ...
    commands.forEach(([command, p1, p2]) => {
        console.log('new command ', command, p1, p2)
        let commandCubes = [[p1, p2] as Cube]
        let [x1, y1, z1] = p1;
        let [x2, y2, z2] = p2;
        if (command == "on") {
            /*for (let x = x1; x <= x2; x++) {
                for (let y = y1; y <= y2; y++) {
                    for (let z = z1; z <= z2; z++) {
                        //arr[x + 50][y + 50][z + 50] = 1;
                    }
                }
            }*/
            //handling on
            for (let existingCube of cubes) {
                for (let i = 0; i < commandCubes.length; i++) {
                    let commandCube = commandCubes[i];
                    if (intersects(existingCube, commandCube)) {
                        //console.log("intersects", commandCube);
                        console.log("=======================");
                        console.log("reducing ", commandCube);
                        let newCommandCubes = getIntersectionOn(existingCube, commandCube);
                        commandCubes.splice(i, 1, ...newCommandCubes)
                        console.log("cubes ", commandCubes);
                        console.log("=======================");
                        //console.log(commandCubes);
                        i--
                    }
                }
            }
            cubes.push(...commandCubes)
        }
        else {
            /*for (let x = x1; x <= x2; x++) {
                for (let y = y1; y <= y2; y++) {
                    for (let z = z1; z <= z2; z++) {
                        //arr[x + 50][y + 50][z + 50] = 0;
                    }
                }
            }*/
            let newCubes = [];
            for (let existingCube of cubes) {
                if (intersects(existingCube, commandCubes[0])) {
                    let rest = getIntersectionOn(commandCubes[0], existingCube);
                    newCubes.push(...rest);
                } else {
        ...
        let sum = calculateSum(cubes);
        let aSum = sum; //arraySum(arr);
        /*if (sum != aSum) {
            diffResults(arr, cubes);
            throw `expected ${aSum} but got ${sum}`;
        }*/
        console.log(`Cubes: ${cubes.length} Lit: ${calculateSum(cubes)} Array: ${arraySum(arr)}`);
    });
    //let sum = arraySum(arr);
    //console.log(sum);
    //return sum;
    return calculateSum(cubes);
}
...
if (part1(input2.slice(0, input2.length - 2)) !== 590784) {
    throw Error('invalid result')
}

part1(input3);
console.log(part1(inputReal) + 2080);
